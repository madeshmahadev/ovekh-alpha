...
interface MessageBroadcasterDisconnected {
    type: "broadcasterdisconnected" | "broadcasterended";
}
...
class Broadcaster implements Client {
    ...
    private screenSharingEnded: boolean = false;
    ...
    */
    private async handleBroadcasterDisconnect(): Promise<void> {
        showPopup("diconnect-sharing");
        //document.body.removeChild(this.videoElement);
        document.body.removeChild(this.counterElement);
        
        // Stop the current screen stream to release resources
        this.mediaStream.getTracks().forEach(track => track.stop());
        this.screenSharingEnded === true;
    }
...
class Room {
    ...
    private setDocumentTitle() {
        ...
        document.title = roomIdWords.join(" ") + " | Ovekh";
    }
    ...
    private async setupBroadcaster(): Promise<Broadcaster> {
        let mediaStream = await this.getDisplayMediaStream();

        const counterElement: HTMLDivElement =
        ...
        participantsIconElement.style.marginRight = "10px"

        const changeScreenIcon = document.createElement('img');
        changeScreenIcon.src = '/translations/changeScreen.svg';
        changeScreenIcon.alt = "Screen Change Icon";
        changeScreenIcon.style.width = "22px";
        changeScreenIcon.style.marginBottom = "-2px";

        const changeScreenBtn = document.createElement('button');
        changeScreenBtn.id = 'changeScreenBtn';
        changeScreenBtn.appendChild(changeScreenIcon);
        changeScreenBtn.addEventListener('click', async () => {
            mediaStream = await this.changeDisplayMediaStream(mediaStream);
        ...
        dividerLine1.className = 'divider';

        const dividerLine2 = document.createElement('div');
        dividerLine2.className = 'divider';

        const disconnectBtn = document.createElement('button');
        disconnectBtn.id = 'endCallBtn';
        ...
        counterElement.appendChild(dividerLine1);
        counterElement.appendChild(changeScreenBtn);
        counterElement.appendChild(dividerLine2);
    ...
    /**
     * Returns the user's display MediaStream when changed.
     *
     * @private
    */
    private async changeDisplayMediaStream(mediaStream: MediaStream): Promise<MediaStream> {
